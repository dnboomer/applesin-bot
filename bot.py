...
async def model_list(update, context):
    ...
    await query.edit_message_text(MESSAGES["choose_model"].format(cat=cat), reply_markup=InlineKeyboardMarkup(kb))
...
async def service_list(update, context):
    ...
    await query.edit_message_text(MESSAGES["choose_service"].format(title=PRICES[cat][mod]['title']), reply_markup=InlineKeyboardMarkup(kb))
...
async def get_contact(update, context):
    ...
    await query.edit_message_text(MESSAGES["get_contact"].format(path=path))
...
async def final_handler(update, context):
    ...
    if 10 <= len(phone) <= 11:
        ...
        await update.message.reply_text(MESSAGES["success"].format(path=context.user_data.get('path'), phone=clean_phone))
    else:
        await update.message.reply_text(MESSAGES["error_phone"])

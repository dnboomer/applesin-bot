...
async def start(update, context):
    ...
    if update.callback_query:
        ...
        await update.callback_query.edit_message_text("Эплсин. Выберите устройство:", reply_markup=InlineKeyboardMarkup(kb))
    else:
...
async def service_list(update, context):
    ...
    # РЕВОЛЮЦИЯ: берем всё после 'mod_', делим только по первому подчерку
    # Например: mod_iPhone_15pro_max -> ['iPhone', '15pro_max']
